...
(defmethod print-method Flake [^Flake f, ^java.io.Writer w]
  (.write w (str "#Flake "))
  (binding [*out* w]
...
(defn- assoc-flake
  ...
  (let [[s p o t op m] (Flake->parts flake)]
    (case k
      ...
      (throw (IllegalArgumentException. (str "Flake does not contain key: " k))))))
...
(comment
  ...
  (def myset (sorted-set-by cmp-flakes-spot-novelty))

  myset

  (def myset2 (conj myset (parts->Flake [123 1 1 -1 true])))
  myset2
  (def myset3 (conj myset2 (parts->Flake [1234 1 1 -1 true])))
  myset3
  (def myset4 (conj myset3 (parts->Flake [123 1 1 -2 false])))
  myset4
  (def myset5 (conj myset4 (parts->Flake [123 1 2 -2 true])))
  myset5
